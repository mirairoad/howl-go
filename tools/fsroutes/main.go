...
func main() {
	var pagesDir, modulePath, out string
	flag.StringVar(&pagesDir, "dir", "client/pages", "root of the page tree")
	flag.StringVar(&modulePath, "module", "howl-go/client/pages", "import path of -dir")
	...
	src, err := render(routes, rootPkg, pagesDir)
	if err != nil {
		log.Fatal(err)
	}
...
func render(routes []route, rootPkg, pagesDir string) ([]byte, error) {
	var b bytes.Buffer
